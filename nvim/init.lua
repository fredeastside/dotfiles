...
vim.keymap.set('n', '<leader>bn', '<cmd>enew<CR>', { desc = '[B]uffer [N]ew scratch buffer' })
...
require('lazy').setup({
  ...
  { -- Fuzzy Finder (files, lsp, etc)
    ...
    event = 'VimEnter',
    -- Tracking master: 0.1.x is frozen and predates nvim 0.10's LSP
    -- deprecations (e.g. make_position_params requires offset_encoding).
    branch = 'master',
    dependencies = {
    ...
    config = function()
      ...
      end, { desc = '[F]ind [D]iagnostics (current file)' })
      vim.keymap.set('n', '<leader>fD', builtin.diagnostics, { desc = '[F]ind [D]iagnostics (all)' })
    ...
    config = function()
      ...
      vim.api.nvim_create_autocmd('LspAttach', {
        ...
        callback = function(event)
          ...
          -- Code action mapped to <leader>la in core/keymaps.lua
      ...
      -- Per-server overrides. Empty table = use lspconfig defaults.
      ...
      -- Register defaults + per-server config via the 0.11 vim.lsp.config API.
    ...
    config = function()
      ...
      cmp.setup {
        ...
        mapping = cmp.mapping.preset.insert {
          -- Select next item or jump forward in snippet
  ...
  { -- Highlight, edit, and navigate code
    ...
    branch = 'main', -- master is EOL; main is the actively maintained line for 0.10+
    lazy = false,
    build = ':TSUpdate',
    config = function()
      ...
      -- Languages to install up front (main has no auto_install).
      require('nvim-treesitter').install {
        'bash', 'c', 'csv', 'dockerfile', 'gitcommit', 'gitignore',
        'go', 'gomod', 'gosum', 'html', 'http', 'javascript', 'json',
        'lua', 'make', 'markdown', 'markdown_inline', 'php', 'proto',
        'ruby', 'rust', 'sql', 'tmux', 'toml', 'tsx', 'typescript',
        'vim', 'vimdoc', 'yaml',
      }

      -- Enable highlight + indent for any filetype whose parser is installed.
